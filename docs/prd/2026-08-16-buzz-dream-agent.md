...
- 输出侧：在 `CommandDrawer` 新增「智能推荐 · 按服务器」分区，在 `HostDetailPanel` 新增「常用命令」卡片；支持采纳（Pin）、编辑、忽略、一键执行；
...
**验收场景**：在真实数据下走通场景 A（查看并执行推荐）、场景 C（空闲自动分析）、场景 D（危险命令确认）、场景 E（无供应商降级）；`pnpm typecheck`、`pnpm test`、`pnpm test:electron` 全绿。
...
- **渲染层**（`tests/renderer/`）：CommandDrawer 推荐分区渲染/搜索/操作；HostDetailPanel 卡片；执行调用注入 fake API；`deterministicDreamApi.ts` 与真实 API 签名对齐。
