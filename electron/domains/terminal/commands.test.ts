...
import { createTerminalCommandHandlers } from "./commands";
import { TerminalRuntime, type PtyProcess } from "./runtime";
...
describe("Electron terminal command handlers", () => {
  it("keeps Electron-owned PTY operations out of the legacy transport", async () => {
    ...
    const context: CommandContext = { streamId: "stream-1", fallback };
  ...
  it("falls back for SSH-owned terminal sessions", async () => {
    ...
    const context: CommandContext = { fallback };
